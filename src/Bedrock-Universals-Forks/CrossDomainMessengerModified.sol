    ...
    OPAddressRegistry_Testnet
{
    ...
    function relayMessage(
        ...
        // Check if the target chain ID is the current chain's block chain ID
        if (_targetChainID == block.chainid) {
            // Proceed with traditional message h
            ...
            if (_isOtherMessenger(_targetChainID)) {
                ...
                require(
                    msg.value == 0, "CrossDomainMessenger: value must be zero unless message is from a system address"
                );
     ...
     */
    function _isOtherMessenger(uint256 _targetChainID) internal view virtual returns (bool);
     ...
     */
    function _isUnsafeTarget(address _target, uint256 _targetChainID) internal view virtual returns (bool);
}
