...
contract L2StandardBridge is StandardBridge, Semver {
    ...
    function finalizeDeposit(
        ...
        if (_l1Token == address(0) && _l2Token == Predeploys.LEGACY_ERC20_ETH) {
            finalizeBridgeETH(CHAINID, _from, _to, _amount, _extraData);
        } else {
            finalizeBridgeERC20(CHAINID, _l2Token, _l1Token, _from, _to, _amount, _extraData);
        }
    }
    ...
    function _initiateWithdrawal(
        ...
        if (_l2Token == Predeploys.LEGACY_ERC20_ETH) {
            _initiateBridgeETH(ETH_CHAINID, _from, _to, _amount, _minGasLimit, _extraData);
        } else {
            address l1Token = OptimismMintableERC20(_l2Token).l1Token();
            _initiateBridgeERC20(ETH_CHAINID, _l2Token, l1Token, _from, _to, _amount, _minGasLimit, _extraData);
        }
    }
